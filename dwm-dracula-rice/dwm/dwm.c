...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isfreesize, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i)
                    drw_text(drw, x, 0, w, bh, lrpad / 2, filledtags[i], urg & 1 << i);
                    /*
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x + bhpad, bhpad, i->w - bhpad * 2, i->h - bhpad * 2);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
